...
try:
	...
	get(userjs("Tea_houses_list.html")).index("*, Tea_types.Name as ttName")
	passert(True)

	pprint("Testing that plot.html contains the given configuration")
	get(userjs("plot.html")).index("Not planting")
	worked = False
	script = scripts("plot.html")
	...
	get(userjs("plot.html")).index("Custom JS run when you search")
	passert(True)
	...
	get(userjs("selects_list.html")).index("Didn't see anything")
	passert(True)
	...
	script = scripts("aa_Tea_houses_detail.html")
	...
	script = scripts("some_graph.html")
